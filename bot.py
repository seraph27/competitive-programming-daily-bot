...
async def set_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    if success:
        await interaction.response.send_message(f"Daily challenges will be posted in {channel.mention}.", ephemeral=True)
    else:
...
async def manual_daily(interaction: discord.Interaction):
    ...
    target = interaction.guild.get_channel(settings["channel_id"]) if settings and interaction.guild.get_channel(settings["channel_id"]) else interaction.channel
    await send_daily_set(target)
    await interaction.followup.send(f"Daily problems posted in {target.mention}", ephemeral=True)
